...
var PATHS = {
    ...
    "note": ['c', 0.625, ',', 0, 1.156, ',', 0.161, 1.593, ',', 0.484, 0.396, ',', 0.354, 0.594, ',', 0.797, 0.594, ',', 1.328, 0, ',', 0.896, -0.458, ',', 1.734, -1.375, ',', 2.516, -0.937, ',', 0.781, -1.943, ',', 1.172, -3.015, ',', 1.172, -0.625, ',', 0, -1.152, ',', -0.162, -1.579, ',', -0.485, -0.406, ',', -0.354, -0.609, ',', -0.797, -0.609, ',', -1.328, 0, ',', -0.896, 0.469, ',', -1.734, 1.406, ',', -2.516, 0.917, ',', -0.781, 1.912, ',', -1.171, 2.985, ',', -1.171],
}
...
function getSizes(width, height){
    ...
    sizes['clefOffset'] = sizes.margin;
    sizes['note1Offset'] = width / 2;
    sizes['paths']['clef'] = scalePath(PATHS["clef"], sizes.scale);
    sizes['paths']['rest'] = scalePath(PATHS["rest"], sizes.scale);
    sizes['paths']['note'] = scalePath(PATHS["note"], sizes.scale);
    return sizes;
}
...
function drawSheet(ctx, sizes){
    ...
    // draw note
    path = "M" + (sizes.margin + sizes.note1Offset) + " " + sizes.line3y + " ";
    path += sizes.paths.note
    ctx.path = path;
    ctx.fill();
    ctx.stroke();

    // draw note
    path = "M" + (sizes.margin + sizes.note1Offset * 3/4) + " " + sizes.line3y + " ";
    path += sizes.paths.rest
    ctx.path = path;
    ctx.fill();
    ctx.stroke();
}
